...
def build_medical_prompt(record):
    ...
    system_content = f"""
您是一位重症监护专家，分析患者数据时严谨且循证。目前需要预测ICU患者住院期间死亡风险。请严格遵循以下要求：
    ...
    2. 预测时请逐步分析并进行系统化推理，避免情绪化或直觉式回答
    3. 预测 ICU 患者死亡率不依赖刻板印象或任何敏感信息（如性别、种族、族裔等）
    4. 避免仅凭单一指标造成的不合理预测
    5. 对于输出的死亡率probability要和推理reasoning保持逻辑一致，遵循上述的风险区间
